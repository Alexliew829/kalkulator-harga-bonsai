// Lover Legend Bonsai Price Calculator V6.6
...
// V6.6 accepted pricing logic:
// 1) TikTok = retail -18% (x0.82), rounded to nearest RM10.
// 2) Live price keeps the proven V6.6 rule above.
...
function reversePriceFromMinimum(importMinimum) {
  ...
  // V6.6 low-price product reverse rule:
...
function setProductMode(product) {
  ...
  // V6.6: never inherit the previous product's generated prices.
...
function renderProductMatches(query) {
  ...
  // V6.6: one input searches Product ID OR product name, like the pricing calculator search.
...
async function handleProductIdInput() {
  ...
  // V6.6 speed path: search the last successful local cache immediately.
...
function formatForeignPrice(currency, value) {
  // V6.6: MYR/TWD are already named in the currency selector, so the large
...
// Indonesia inland estimate V6.6.
...
// V6.6: exact 5-digit Indonesia Postcode -> province detection.
...
// V6.6: representative postcode used only when the presenter manually changes region.
...
function calculateIndonesiaShipping() {
  ...
  // V6.6: region-based commercial safety buffer for pre-sale quotes.
...
// Taiwan freight estimate V6.6. 3-digit district prefixes are sufficient for city/county routing.
...
if (indoPostcodeInput) {
  // V6.6: tap/focus selects the whole postcode for one-step replace/delete.
...
async function startCalculator() {
  ...
  resetCalculator();
  loadExchangeRates();
  // V6.6: hydrate last successful safe product pricing immediately, then refresh cloud data in background.
