// Lover Legend Bonsai Price Calculator V5.6
...
// V5.6 accepted pricing logic:
// 1) TikTok = retail -18% (x0.82), rounded to nearest RM10.
// 2) Live price keeps the proven V5.6 rule above.
...
function reversePriceFromMinimum(importMinimum) {
  ...
  // V5.6 low-price product reverse rule:
...
function setProductMode(product) {
  ...
  // V5.6: never inherit the previous product's generated prices.
...
function renderProductMatches(query) {
  ...
  // V5.6: one input searches Product ID OR product name, like the pricing calculator search.
...
async function handleProductIdInput() {
  ...
  try {
    ...
    // V5.6: a complete valid PZ typed directly must behave exactly like clicking a dropdown result.
...
// Indonesia inland estimate V5.6.
...
// V5.6: exact 5-digit Indonesia Postcode -> province detection.
...
// V5.6: representative postcode used only when the presenter manually changes region.
...
function calculateIndonesiaShipping() {
  ...
  // V5.6: region-based commercial safety buffer for pre-sale quotes.
...
// Taiwan freight estimate V5.6. 3-digit district prefixes are sufficient for city/county routing.
...
if (indoPostcodeInput) {
  // V5.6: tap/focus selects the whole postcode for one-step replace/delete.
