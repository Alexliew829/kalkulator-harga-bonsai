// Lover Legend Bonsai Price Calculator V6.2
...
// V6.2 accepted pricing logic:
// 1) TikTok = retail -18% (x0.82), rounded to nearest RM10.
// 2) Live price keeps the proven V6.2 rule above.
...
function reversePriceFromMinimum(importMinimum) {
  ...
  // V6.2 low-price product reverse rule:
...
function setProductMode(product) {
  ...
  // V6.2: never inherit the previous product's generated prices.
...
async function loadProductPricing(forceRetry) {
  ...
  productPricingLoading = (async function () {
    let rawProducts;
    let lastError;
    // V6.2: Apps Script can occasionally be cold/transient. Retry once before showing unavailable.
...
function renderProductMatches(query) {
  ...
  // V6.2: one input searches Product ID OR product name, like the pricing calculator search.
...
async function handleProductIdInput() {
  ...
  try {
    ...
    // V6.2: a complete valid PZ typed directly must behave exactly like clicking a dropdown result.
    // V6.2: a real current ID always wins over a historical alias. This keeps
    // genuine PS flowerpot IDs safe if an old alias ever shares the same text.
...
function formatForeignPrice(currency, value) {
  // V6.2: MYR/TWD are already named in the currency selector, so the large
...
// Indonesia inland estimate V6.2.
...
// V6.2: exact 5-digit Indonesia Postcode -> province detection.
...
// V6.2: representative postcode used only when the presenter manually changes region.
...
function calculateIndonesiaShipping() {
  ...
  // V6.2: region-based commercial safety buffer for pre-sale quotes.
...
// Taiwan freight estimate V6.2. 3-digit district prefixes are sufficient for city/county routing.
...
if (indoPostcodeInput) {
  // V6.2: tap/focus selects the whole postcode for one-step replace/delete.
