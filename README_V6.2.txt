...
- GitHub Pages 需完整覆盖 V6.2 文件；Import V16.4 无需再次修改或重新部署。

V6.2 产品编号兼容：旧 PZxxxx 与 Import V16.4 新前缀编号可查询同一产品；匹配同时核对产品名称和原四位数字，最低售价仍使用当前产品资料。
...
部署：
- Calculator: GitHub Pages 覆盖全部 V6.2 文件。
...
V6.2 修正：MYR/TWD 大金额不重复货币符号；配合 Import V16.4 有效最低售价接口。
...
V6.2 排版调整：
- 以 V6.1 为基准，只调整产品输入区块排版，不改价格、搜索、运费、汇率或最低售价逻辑。
- 「产品编号 / Product ID」与「BX / 产品」搜索框改为同一行。
- 搜索框下方完整显示「BX0004 · 产品名称」，不再以省略号截断；产品编号字体略小。
- 产品输入区块右侧维持「直播售价 / Harga Live」与金额，左右区块重新分配宽度，兼顾完整产品名和直播售价。
