...
- GitHub Pages 需完整覆盖 V6.4 文件；Import V16.4 无需再次修改或重新部署。

V6.4 产品编号兼容：旧 PZxxxx 与 Import V16.4 新前缀编号可查询同一产品；匹配同时核对产品名称和原四位数字，最低售价仍使用当前产品资料。
...
部署：
- Calculator: GitHub Pages 覆盖全部 V6.4 文件。
...
V6.4 修正：MYR/TWD 大金额不重复货币符号；配合 Import V16.4 有效最低售价接口。
...
V6.4 product search speed optimization:
...
V6.4 排版调整：
- 仅调整产品/直播售价区域排版，不修改任何计算或搜索逻辑。
- 直播售价 / Harga Live 改为单行显示。
- 直播售价移除重复 RM 前缀。
- 产品编号输入区加宽；下方产品名称允许完整换行显示。
